...
export default function Admin() {
  ...
  async function load() {
    ...
    try {
      ...
      setItems(data.results); setTotal(data.total);
  ...
  async function changeStatus(id, newStatus) {
    try {
      ...
      setActionMsg({ type: "error", text: e?.response?.data?.error || "Failed to update" });
    } finally { setTimeout(() => setActionMsg(null), 1200); }
  }
  ...
  async function addComment() {
    ...
    try {
      ...
      setNewComment(""); await loadComments(sel.id);
    } catch {
    ...
    <div className="center-page">
      <div className="stack">
        <div className="card card-full">
          ...
          {loading ? <div className="p" style={{textAlign:"center"}}>Loading…</div> : (
                    ...
                    <td className="td" style={{textAlign:"center"}}>
                      <div style={{display:'flex', gap:8, flexWrap:'wrap', justifyContent:'center'}}>
                        {isAdmin && STATUS.map(s => (
                          <button key={s} className="btn btn-secondary" style={{padding:'8px 10px'}} onClick={()=> changeStatus(row.id, s)}>
                            {s.replace('_',' ')}
                          </button>
                        ))}
                        <button className="btn" style={{padding:'8px 10px'}} onClick={() => { setSel(row); loadComments(row.id); }}>
                          View thread
                        </button>
                      </div>
                    </td>
            ...
            <>
              ...
              {isAdmin && (
                <div className="row" style={{marginBottom:10}}>
                  <textarea className="textarea" placeholder="Add internal note… (admin only)"
